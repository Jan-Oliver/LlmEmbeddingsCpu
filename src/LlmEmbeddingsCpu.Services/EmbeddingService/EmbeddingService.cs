...
namespace LlmEmbeddingsCpu.Services.EmbeddingService
{
    public class EmbeddingService : IEmbeddingService, IDisposable
    {
        ...
        private readonly int _embeddingSize = 384; // Default for MiniLM-L6-v2
        private readonly string _modelName;
        private readonly string _modelDirectory;

        public EmbeddingService(string modelName = "all-MiniLM-L6-v2", bool throwIfModelMissing = false)
        {
            _modelName = modelName;
            
            // Look for models in the deps directory
            _modelDirectory = Path.Combine(GetRootDirectory(), "deps", "sentence-transformers", modelName);
